...
export function buildDashboardAiBrief(
  ...
  const { watchlist, portfolioSentiment, buzzSurgeTop3, kospiSentiment, marketOutlierRows } =
    overview
  const { isLoggedIn } = options

  if (!isLoggedIn) {
    const alerts = pickDashboardAlerts(marketOutlierRows, 3, 'market')
    const stockNames = alerts.map((alert) => alert.name)
